...
export class Snapshot extends React.Component {
  ...
  render () {
    return (
      <div className={classnames('snapshot', _get(this.props, 'overview.snap.meta.isLoading') ? 'loading-overlay' : '')}>
        <div className={classnames('row__blue')}>
          <h2 className={classnames('row__title')}>Performance Snapshot</h2>
          ...
          <div className={classnames('stat__row')}>
            {this.props.children}
          </div>
        </div>
      </div>
